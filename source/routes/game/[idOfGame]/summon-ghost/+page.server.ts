...
import {z} from "zod";
const schemaForInputData = z.strictObject({targetId: z.string().nonempty()});
export const actions = {
	default: async function summonGhost(
		event: RequestEvent,
		...
		return await server_.handlingRequests_.handleActionWithData(
			...
			function do_(
				...
				context.storageOfGame.summonGhost(context.player.id, data.targetId);
				return;
			},
		);
	},
